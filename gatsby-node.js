...
exports.onPostBuild = (_, pluginOptions) => {
  ...
  const { files, publicPath, gaConfigPath, gtmContainerId, dist, serviceWorker } = { ...defaultOptions, ...pluginOptions }
  const absolutePaths = files.map(file => path.join(process.cwd(), publicPath, file))
  const htmls = globby.sync(absolutePaths)
  const config = { gaConfigPath, gtmContainerId, cwd: path.join(process.cwd(), publicPath), serviceWorker }
